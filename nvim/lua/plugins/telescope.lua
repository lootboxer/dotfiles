    ...
    vim.keymap.set('n', '<leader>smp', function()
      ...
      local function is_git_ignored(filepath)
        ...
        if vim.v.shell_error ~= 0 then
          return false -- Not a git repo, don't filter
        end

        -- Use git check-ignore to see if the file is ignored
        local result = vim.fn.system('git -C ' .. vim.fn.shellescape(project_root) .. ' check-ignore -q ' .. vim.fn.shellescape(filepath))
        return vim.v.shell_error == 0 -- Returns 0 if file is ignored
      end
